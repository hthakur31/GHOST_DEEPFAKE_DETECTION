...
class FaceForensicsDetector:
    ...
    def __init__(self, model_path: Optional[str] = None, device: str = 'auto', threshold: float = 0.5):
        ...
        # Auto-find trained model if not specified
        if model_path is None:
            ...
            if os.path.exists(models_dir):
                ...
                if model_files:
                    ...
                    logger.info(f"Auto-detected trained model: {model_path}")
    ...
    def detect_video(self, video_path: str) -> Dict:
        ...
        try:
            ...
            with torch.no_grad():
                ...
                # Simple threshold-based decision (slightly lower threshold for better deepfake detection)
                threshold = 0.48  # Slightly lower threshold to catch edge cases
                
                # Direct comparison: if fake score > threshold, predict fake
                if weighted_fake_score > threshold:
